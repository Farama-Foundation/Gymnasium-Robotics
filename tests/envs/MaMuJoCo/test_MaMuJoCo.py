...
def test_action_and_observation_mapping(observation_depth, task):
    ...
    """
    test_env = MultiAgentMujocoEnv(task.scenario, task.conf, agent_obsk=observation_depth)
...
def test_k_dict():
    ...
    scenario = "Ant"
    agent_conf = "2x4"
    test_env = MultiAgentMujocoEnv(scenario=scenario, agent_conf=agent_conf, agent_obsk=0)
    assert (
        str(test_env.k_dicts)
        == "[{0: [ankle1, ankle2, hip1, hip2]}, {0: [ankle3, ankle4, hip3, hip4]}]"
    ), "wrong k_dicts: " + str(test_env.k_dicts)
    test_env = MultiAgentMujocoEnv(scenario=scenario, agent_conf=agent_conf, agent_obsk=1)
    assert (
        str(test_env.k_dicts)
        == "[{0: [ankle1, ankle2, hip1, hip2], 1: [hip3, hip4]}, {0: [ankle3, ankle4, hip3, hip4], 1: [hip1, hip2]}]"
    ), "wrong k_dicts: " + str(test_env.k_dicts)
    test_env = MultiAgentMujocoEnv(scenario=scenario, agent_conf=agent_conf, agent_obsk=2)
    assert (
        str(test_env.k_dicts)
