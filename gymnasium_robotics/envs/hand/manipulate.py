...
class MujocoHandBlockEnv(MujocoManipulateEnv):
    ...
    The environment is based on the [Shadow Dexterous Hand](https://www.shadowrobot.com/), which is an antropomorphic robotic hand with 24 joints. In this task a block is placed on the palm of the hand. The task is to then manipulate the
    ...
    * `HandManipulateBlockFull-v1`: Random target rotation for all axes of the block. Random target position.
    ...
    In the robot hand an extra joint is added to the little finger `LFJ4` in order to perform the opposition movement with the thumb. Also the the human thumb has two different joints than the rest of the fingers. The *carpometacarpal* (CMC) joint located
    close to the palm area, `THJ4` and `THJ3`in the robot. And the *interphalangeal* joint which is in the same location as the DIP but in this case actuated. This joint is the `THJ0` in the robot hand.
