...
class MujocoFetchPickAndPlaceEnv(MujocoFetchEnv, EzPickle):
    ...
    The task in the environment is for a manipulator to move a block to a target position on top of a table or in mid-air. The robot is a 7-DoF [Fetch Mobile Manipulator](https://www.robotandchisel.com/assets/papers/Fetch-and-Freight-Workshop-Paper.pdf) with a two-fingered parallel gripper.
    ...
    * `achieved_goal`: this key represents the current state of the block, as if it would have achieved a goal. This is useful for goal orientated learning algorithms such as those that use [Hindsight Experience Replay](https://arxiv.org/abs/1707.01495) (HER). The value is an `ndarray` with shape `(3,)`. The elements of the array are the following:
    ...
    To initialize this environment with one of the mentioned reward functions the type of reward must be specified in the id string when the environment is initialized. For `sparse` reward the id is the default of the environment, `FetchPickAndPlace-v4`. However, for `dense` reward the id must be modified to `FetchPickAndPlaceDense-v4` and initialized as follows:
