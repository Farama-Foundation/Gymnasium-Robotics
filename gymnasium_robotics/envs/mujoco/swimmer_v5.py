...
class SwimmerEnv(MujocoEnv, utils.EzPickle):
    ...
    * θ<sub>i</sub>': its derivative with respect to time (angular velocity)
    ...
    ## Version History
    * v5: All MuJoCo environments now use the MuJoCo bindings in mujoco >= 2.3.3. The `xml_file` argument is re-added, replaced "reward_fwd"/ "forward_reward" with "reward_forward" in `info`.
